			...
			if(!isLoading) {
				...
				Task.Run(() => {
					...
					foreach(var song in
						...
						.SelectMany(x => x.beatmapLevels.Where(l => !l.hasPrecalculatedData))
					) {
						if(songTimes.ContainsKey(song.levelID) && !fullReload)
							continue;

						var customLevelPath = SongCore.Collections.GetCustomLevelPath(song.levelID);
						 ...
						 */
						songTimes[song.levelID] = (int)File.GetCreationTimeUtc(customLevelPath + fpath).ToUnixTime();
					}
